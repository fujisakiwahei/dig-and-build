...
- [ ] Astroのトランジションで、ページ遷移の際にフェードを入れた。この時、View Transitionだとページ遷移してもURLが変わらないためGA4の計測などに問題が出る。なのでリロードを明示した。この辺り復習する。
...
- GA4でプロパティを作った
- GA4のタグを変わった方法で記述。以下でフロントマター内で変数にして共通レイアウトのheadの下に出力。その際エスケープされないように工夫が必要。
